...
> **⚠ Implementation note (Phase 8 H1 fix, 2026-05-02):** When this gate is implemented in code, the "4+ same-direction with 0 opposing" tally MUST exclude the trend vote itself — otherwise the gate is mathematically unreachable in any uptrend (the trend always contributes +1 to the bullish count). Concretely: in any uptrend, the gate condition for shorts evaluates to `bearish ≥ 4 and bullish == 0`, but `bullish ≥ 1` always (because trend = bullish), so shorts can never fire. The pre-Phase-8 code shipped this bug for several months; it was caught by the goldtest replay harness (0/7 short pass rate). The fix is to compute `bullish_excl_trend` / `bearish_excl_trend` and use those in the gate condition only — keeping the candidate-selection logic on the full tally. New threshold: **3-of-4 non-trend agree, 0 oppose** (preserves the "overwhelming non-trend consensus to override the trend" intent without the impossibility). See `BP_rules_engine.py _bias_consensus` (search for `Phase 8 H1 fix`).
